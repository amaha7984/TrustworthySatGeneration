...
def init_net(net, init_type='normal', init_gain=0.02, gpu_ids=[], debug=False, initialize_weights=True):
    ...
    if len(gpu_ids) > 0 and gpu_ids[0] != -1:
        assert(torch.cuda.is_available()), "CUDA is not available, but GPU IDs are specified."
        net.to(torch.device('cuda:{}'.format(gpu_ids[0])))
        net = torch.nn.DataParallel(net, gpu_ids)  # multi-GPU support
    else:
        # Print a message if no GPU is used
        print("No GPU IDs specified or running on CPU. Using CPU for inference.")
        net.to(torch.device('cpu'))  # Use CPU instead
